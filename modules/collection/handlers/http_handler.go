...
func InitCollectionHttpHandler(router *gin.Engine) {
	router.GET("/collection/v1/:user_id", middlewares.VerifyToken, GetCollectionByUserID)
	router.POST("/collection/v1", middlewares.VerifyToken, CreateCollection)
	router.PUT("/collection/v1/:id", middlewares.VerifyToken, UpdateCollection)
	router.DELETE("/collection/v1/:id", middlewares.VerifyToken, DeleteCollection)
}
